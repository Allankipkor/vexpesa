    ...
    if (stake > 0 && db && username) {
      try {
        ...
        if (users.length > 0) {
          ...
          if (tradeRef) {
            try {
              await db`UPDATE zentrapesa_trades SET result = 'cancelled', resolved_at = CURRENT_TIMESTAMP WHERE trade_ref = ${tradeRef}`;
            } catch(e) {}
          }
